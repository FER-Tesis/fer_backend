...
from app.api.emotion_alert_ws import router as emotion_alert_ws_router
...
from app.realtime.emotion_alert_realtime_listener import emotion_alert_realtime_listener
...
async def startup_event():
    ...
    await emotion_alert_realtime_listener.start()

@app.on_event("shutdown")
async def shutdown_event():
    await camera_alert_listener.stop()
    await emotion_alert_listener.stop()
    await emotion_alert_realtime_listener.stop()
...
app.include_router(emotion_alert_ws_router, prefix="/api/emotion/alert/ws", tags=["emotion_alerts_ws"])